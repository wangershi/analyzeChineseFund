...
def analyzeHistoricalValue(ifUseNewIssues = True, ifUseOldIssues = True, ifUseWatchList = False, ifUseAdjustFactorToLatestDay = False, ifPrintFundCode=False):
    ...
    # TODO: use latest fund information
    ...
    fundNameListForOldIssues = []
    riskListForNewIssues = []
    returnListForNewIssues = []
    fundNameListForNewIssues = []
    for file in os.listdir(rootFolder):
        ...
        if count % 100 == 0:
            print ("\ncount = %s\tfundCode = %s\tfundName = %s" % (count, fundCode, fundName))  # 180003 银华-道琼斯88指数A
        try:
                ...
                if ifUseAdjustFactorToLatestDay:
                    if countNetValue < daysRangeToAnalyze:
                        ...
                        try:
                            ...
                            #print ("adjustedFactor = %s" % adjustedFactor)  # 0.987561058590916
    ...
    if ifPrintFundCode:
        if ifUseOldIssues:
            for i in range(len(fundNameListForOldIssues)):
                x = riskListForOldIssues[i]
                y = returnListForOldIssues[i]
                fundName = fundNameListForOldIssues[i]
                plt.text(x, y, fundName, fontsize=10)

        if ifUseNewIssues:
            for i in range(len(fundNameListForNewIssues)):
                x = riskListForNewIssues[i]
                y = returnListForNewIssues[i]
                fundName = fundNameListForNewIssues[i]
                plt.text(x, y, fundName, fontsize=10)
...
def getCorrelationMatrixForOneFund(ifGetCorrFromFile = True, ifGetDfMergeFromFile = True):
    ...
    plt.savefig("./data/correlation_%s.png" % nameFund)
